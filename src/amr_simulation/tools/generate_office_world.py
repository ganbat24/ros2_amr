...
def build_sdf():
    """Return the complete amr_office world SDF document."""
...
def build_map_yaml():
    """Return the map yaml (image, resolution, origin, thresholds)."""
    width = int((WORLD_X + 2 * MARGIN) / RES)
    height = int((WORLD_Y + 2 * MARGIN) / RES)
    return (
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        help='Directory for amr_office.pgm/.yaml '
             '(default: amr_navigation/maps)',
    )
