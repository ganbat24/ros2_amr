...
def generate_test_description():
    ...
    rsp_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            "robot_description": os.path.join(amr_description_pkg, "urdf", "amr.urdf.xacro")
        }.items(),
    )
...
class TestAmrDescriptionLaunch(unittest.TestCase):
    def test_tf_topic_exists(self):
        ...
        self.assertTrue(wait.wait(), "Expected topics not found within timeout")

    def test_xacro_processes(self):
        amr_description_pkg = get_package_share_directory("amr_description")
        xacro_path = os.path.join(amr_description_pkg, "urdf", "amr.urdf.xacro")
